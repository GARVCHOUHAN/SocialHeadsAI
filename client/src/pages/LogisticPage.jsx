...
export const LogisticsPage = () => {
    ...
    return (
        <div className="space-y-8">
            <h1 className="text-4xl font-bold text-white">Logistics</h1>
            <p className="text-gray-400">Track your merchandise shipments and orders.</p>

            {/* Summary */}
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <Card>
                    <h3 className="text-gray-400 text-sm font-medium">Active Shipments</h3>
                    <p className="text-3xl font-bold text-white mt-2">{logisticsData.filter(d => d.status === 'Shipped').length}</p>
                </Card>
                 <Card>
                    <h3 className="text-gray-400 text-sm font-medium">Processing Orders</h3>
                    <p className="text-3xl font-bold text-yellow-400 mt-2">{logisticsData.filter(d => d.status === 'Processing').length}</p>
                </Card>
                <Card>
                    <h3 className="text-gray-400 text-sm font-medium">Delivered (Last 30d)</h3>
                    <p className="text-3xl font-bold text-green-400 mt-2">{logisticsData.filter(d => d.status === 'Delivered').length}</p>
                </Card>
            </div>
                    ...
                    <table className="w-full text-left">
                        <thead className="border-b border-gray-700">
                            <tr>
                                ...
                                <th className="p-3 text-sm font-semibold text-gray-400">Tracking</th>
                            </tr>
                        </thead>
                        <tbody>
                            {logisticsData.map(order => (
                                <tr key={order.id} className="border-b border-gray-800 hover:bg-gray-700/50">
                                    <td className="p-3 text-gray-300 font-mono">{order.id}</td>
                                    <td className="p-3 text-white">{order.customer}</td>
                                    <td className="p-3 text-gray-300">{order.item}</td>
                                    <td className="p-3">
                                        <span className={`px-2 py-1 text-xs font-semibold rounded-full ${getStatusChip(order.status)}`}>
                                            {order.status}
                                        </span>
                                    </td>
                                    <td className="p-3 text-blue-400 font-mono">{order.tracking || 'N/A'}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
